...
def add_del_item_record():
    ...
    add_del_item_records_appl.title('ASK BILLING S/W | ITEM-MANAGEMENT WINDOW ')
    ...
    def insert_record():
        with open('C://ASK-BILLING-S-W//ADD_DEL_ITEMS//row_id.txt','r') as row_id_file:
            row_id=int(row_id_file.read())
        with open('C://ASK-BILLING-S-W//ADD_DEL_ITEMS//item_records.csv','a',newline='') as file:
            file_writer=csv.writer(file)
            tree.insert('',row_id,text='ITEM_'+str(row_id),values=(item_name_entry.get(),'Rs. '+str(item_price_entry.get())))
            item_record=row_id,item_name_entry.get(),item_price_entry.get(),1
            file_writer.writerow(item_record)
        with open('C://ASK-BILLING-S-W//ADD_DEL_ITEMS//row_id.txt','w') as row_id_file:
            row_id_file.write(str(row_id+1))
